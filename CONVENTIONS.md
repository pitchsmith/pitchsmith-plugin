...
| Command | Plugin Workflow | Override Location | Description |
|---------|----------------|-------------------|-------------|
| `/pitchsmith:plan` | `${CLAUDE_PLUGIN_ROOT}/workflows/plan/` | `.slide-builder/workflows/plan/` | Smart router with context-aware routing (single, deck, or continue) |
| `/pitchsmith:plan-one` | `${CLAUDE_PLUGIN_ROOT}/workflows/plan-one/` | `.slide-builder/workflows/plan-one/` | Plan a single slide |
| `/pitchsmith:plan-deck` | `${CLAUDE_PLUGIN_ROOT}/workflows/plan-deck/` | `.slide-builder/workflows/plan-deck/` | Plan a full deck |
| `/pitchsmith:build-one` | `${CLAUDE_PLUGIN_ROOT}/workflows/build-one/` | `.slide-builder/workflows/build-one/` | Build next/single slide |
| `/pitchsmith:build-all` | `${CLAUDE_PLUGIN_ROOT}/workflows/build-all/` | `.slide-builder/workflows/build-all/` | Build all remaining slides |
| `/pitchsmith:edit` | `${CLAUDE_PLUGIN_ROOT}/workflows/edit/` | `.slide-builder/workflows/edit/` | Edit slide layout |
| `/pitchsmith:add-slide` | `${CLAUDE_PLUGIN_ROOT}/workflows/add-slide/` | `.slide-builder/workflows/add-slide/` | Add a new slide to existing deck |
| `/pitchsmith:animate` | `${CLAUDE_PLUGIN_ROOT}/workflows/animate/` | `.slide-builder/workflows/animate/` | Generate animation groups |
| `/pitchsmith:use-template` | `${CLAUDE_PLUGIN_ROOT}/workflows/use-template-deck/` | `.slide-builder/workflows/use-template-deck/` | Instantiate a deck template |
| `/pitchsmith:refresh` | N/A (no workflow) | N/A | Regenerate viewer/manifest |
...
| Command | Plugin Workflow | Override Location | Description |
|---------|----------------|-------------------|-------------|
| `/pitchsmith:add-slide-template` | `${CLAUDE_PLUGIN_ROOT}/workflows/add-slide-template/` | `.slide-builder/workflows/add-slide-template/` | Create new slide template |
| `/pitchsmith:add-deck-template` | `${CLAUDE_PLUGIN_ROOT}/workflows/add-deck-template/` | `.slide-builder/workflows/add-deck-template/` | Create new deck template |
| `/pitchsmith:edit-deck-template` | `${CLAUDE_PLUGIN_ROOT}/workflows/edit-deck-template/` | `.slide-builder/workflows/edit-deck-template/` | Edit existing deck template |
| `/pitchsmith:update-brand-assets` | `${CLAUDE_PLUGIN_ROOT}/workflows/update-brand-assets/` | `.slide-builder/workflows/update-brand-assets/` | Manage brand asset catalog (icons, logos, images) |
| `/pitchsmith:delete-deck` | `${CLAUDE_PLUGIN_ROOT}/workflows/delete-deck/` | `.slide-builder/workflows/delete-deck/` | Delete a deck and files |
| `/pitchsmith:optimize-instructions` | `${CLAUDE_PLUGIN_ROOT}/workflows/optimize-instructions/` | `.slide-builder/workflows/optimize-instructions/` | Optimize workflow instructions |
| `/pitchsmith:eject-workflow` | `${CLAUDE_PLUGIN_ROOT}/workflows/eject-workflow/` | `.slide-builder/workflows/eject-workflow/` | Copy a plugin workflow to project for customization |
| `/pitchsmith:list-overrides` | `${CLAUDE_PLUGIN_ROOT}/workflows/list-overrides/` | `.slide-builder/workflows/list-overrides/` | Show which workflows use defaults vs local overrides |
...
| Command | Plugin Workflow | Override Location | Description |
|---------|----------------|-------------------|-------------|
| `/pitchsmith:setup` | `${CLAUDE_PLUGIN_ROOT}/workflows/setup/` | `.slide-builder/workflows/setup/` | Create brand theme from assets |
| `/pitchsmith:theme` | `${CLAUDE_PLUGIN_ROOT}/workflows/theme/` | `.slide-builder/workflows/theme/` | Display current theme summary |
| `/pitchsmith:theme-edit` | `${CLAUDE_PLUGIN_ROOT}/workflows/theme-edit/` | `.slide-builder/workflows/theme-edit/` | Modify existing theme |

### Meta Commands (/pitchsmith:)
Help and discovery

| Command | Plugin Workflow/Skill | Override Location | Description |
|---------|----------------------|-------------------|-------------|
| `/pitchsmith` | `${CLAUDE_PLUGIN_ROOT}/skills/pitchsmith/SKILL.md` | N/A | Smart entry point - detects context and routes to appropriate workflow |
| `/pitchsmith:help` | `${CLAUDE_PLUGIN_ROOT}/commands/pitchsmith/help.md` | N/A | Show all commands organized by namespace |
| `/pitchsmith:status` | `${CLAUDE_PLUGIN_ROOT}/workflows/status/` | `.slide-builder/workflows/status/` | Display unified slide queue status dashboard |
...
## Override-First Workflow Resolution Pattern

All commands implement an override-first resolution pattern that allows power users to customize workflows while defaulting to plugin-distributed versions.

### Resolution Order

For every command that invokes a workflow:

1. **Check Project Override First**: Check if `.slide-builder/workflows/{workflow-name}/instructions.md` exists in the project
2. **Fallback to Plugin Default**: If not found, use `${CLAUDE_PLUGIN_ROOT}/workflows/{workflow-name}/` from the plugin
3. **Error if Neither Exists**: Display error "Workflow '{workflow-name}' not found in plugin or user overrides"

### Path Variable Reference

| Variable | Resolved By | Points To | Example |
|----------|-------------|-----------|---------|
| `${CLAUDE_PLUGIN_ROOT}` | Claude Code plugin runtime | Plugin installation directory | `~/.claude/plugins/pitchsmith/` |
| `{installed_path}` | Workflow YAML | `${CLAUDE_PLUGIN_ROOT}/workflows/{name}` | `${CLAUDE_PLUGIN_ROOT}/workflows/build-one` |
| `.slide-builder/` | Project-relative | User state directory | `./my-project/.slide-builder/` |

### Logging Format

When a command resolves a workflow path, the resolution source is communicated in the conversation output:

- **Local override used**: `[Workflow] Using local override: .slide-builder/workflows/{name}/`
- **Plugin default used**: `[Workflow] Using plugin default: ${CLAUDE_PLUGIN_ROOT}/workflows/{name}/`

### Command File Example

```markdown
<!-- In command file (e.g., commands/pitchsmith/build-one.md) -->
1. Resolve workflow path (override-first pattern):
   - Check if @.slide-builder/workflows/build-one/instructions.md exists
   - If yes: Use @.slide-builder/workflows/build-one/ as workflow root
   - If no: Use @${CLAUDE_PLUGIN_ROOT}/workflows/build-one/ as workflow root
   - If neither exists: Display error "Workflow 'build-one' not found in plugin or user overrides"
...
### Framework vs State Path Separation

All paths in the plugin follow a strict two-tier model:

| Path Type | Variable | Mutability | Examples |
|-----------|----------|------------|---------|
| **Framework** (commands, workflows, scripts, templates) | `${CLAUDE_PLUGIN_ROOT}` or `{installed_path}` | Immutable (replaced on plugin update) | `${CLAUDE_PLUGIN_ROOT}/workflows/build-one/instructions.md`, `${CLAUDE_PLUGIN_ROOT}/scripts/generate-manifest.js` |
| **State** (theme, catalogs, status, output) | Project-relative paths | User-owned (never overwritten by plugin) | `.slide-builder/config/theme.json`, `.slide-builder/status.yaml`, `output/` |

This separation ensures plugin updates never overwrite user state, and user customizations never block framework updates.

### Ejecting Workflows

Use `/pitchsmith:eject-workflow {workflow-name}` to copy a plugin workflow to your project for customization. Once ejected, your local version will be used for all future invocations due to the override-first resolution pattern.

To revert to plugin defaults, simply delete the local directory:
```bash
rm -rf .slide-builder/workflows/{workflow-name}/
```

Use `/pitchsmith:list-overrides` to see which workflows are ejected vs using plugin defaults.

See also: Epic P1-3 Story 3.1 for the full eject workflow feature specification.

## Workflow Execution Pattern

All workflows follow the BMAD pattern:

1. **Load** `workflow.yaml` from the resolved workflow directory (override or plugin)
...
### Plan Router Context-Aware Routing

The `/pitchsmith:plan` command uses a context-aware router that reads the deck registry to determine available options. The routing behavior adapts based on whether in-progress decks exist.

**Deck Registry Reading:**
- Reads `.slide-builder/status.yaml` and parses the `decks:` section
- Filters for in-progress decks: status = `planned` or `building` (NOT `complete`)
- For each matching deck, extracts: `name`, `status`, `total_slides`, `built_count`, `output_folder`

**Routing Decision Table:**

| Deck Registry State | Choices Presented | Context Text |
|---------------------|-------------------|--------------|
| In-progress decks exist (planned/building) | 3 choices: "Continue deck" / "New deck" / "Single slide" | Shows deck summary with names, status, and slide progress |
| No in-progress decks (empty, all complete, or missing) | 2 choices: "Full deck" / "Single slide" | No deck summary shown |

**Deck Summary Format:**
```
You have N deck(s) in progress:
- **Deck Name** (status) -- X/Y slides built
```

**Continue Deck Selection:**
- If 1 in-progress deck: auto-selects (no additional prompt)
- If multiple in-progress decks: presents a choice list with deck names and progress stats
- Selected deck's `plan.yaml` is loaded from `{{output_folder}}/plan.yaml`
- Routes to plan-deck with `continue_from_phase=5` (modification loop)
...
- If `status.yaml` is missing, cannot be read, or YAML parse fails: falls back to standard 2-choice routing
- If `decks:` key is missing: falls back to standard 2-choice routing
- If `plan.yaml` cannot be read for a selected deck: reports issue and invokes plan-deck fresh
- Never fails silently -- if deck registry is corrupted, informs user but continues with degraded functionality
...
Phase 4 (Section Goal Discovery) incorporates `discovery_context` when generating section goals. The enrichment is LLM reasoning — not mechanical variable substitution. When discovery context is available, the LLM weaves specific insights into more targeted, actionable goals:

| Discovery Key | Goal Field(s) Enriched | How It Enriches |
|---------------|----------------------|-----------------|
| `audience_role` | `communication_objective`, `audience_takeaway` | Orients objective toward role-specific decision language (e.g., CFO → ROI framing, engineer → capability proof) |
| `audience_priorities` | `communication_objective`, `audience_takeaway` | Aligns section objectives and takeaways directly to stated priorities |
| `known_objections` | `content_requirements` | Specifies evidence, data, or counterarguments that directly address each objection |
| `competitive_context` | `communication_objective`, `content_requirements` | Frames differentiation in problem/solution sections; requires comparative data in content |
| `decision_timeline` | `communication_objective` (CTA sections) | Injects urgency and timeline-aware calls to action |

When `discovery_context` is absent or empty (e.g., plans created before Epic AD-1), Phase 4 generates goals using standard inputs only (`narrative_role`, `purpose`, `audience`, `key_points`) — identical to pre-Epic-AD-1 behavior.
