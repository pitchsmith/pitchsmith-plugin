...
1. **Resolve workflow path** (override-first pattern):
   - Check if @.slide-builder/workflows/delete-deck/instructions.md exists
   - If yes: Use @.slide-builder/workflows/delete-deck/ as workflow root
   - If no: Use @${CLAUDE_PLUGIN_ROOT}/workflows/delete-deck/ as workflow root
   - If neither exists: Display error "Workflow 'delete-deck' not found in plugin or user overrides"
2. Read the workflow configuration at {workflow_root}/workflow.yaml
3. Read the instructions at {workflow_root}/instructions.md
4. Execute the 4-step workflow:
