...
1. **Resolve workflow path** (override-first pattern):
   - Check if @.slide-builder/workflows/build-all/instructions.md exists
   - If yes: Use @.slide-builder/workflows/build-all/ as workflow root
   - If no: Use @${CLAUDE_PLUGIN_ROOT}/workflows/build-all/ as workflow root
   - If neither exists: Display error "Workflow 'build-all' not found in plugin or user overrides"
...
5. All generated slides must have:
