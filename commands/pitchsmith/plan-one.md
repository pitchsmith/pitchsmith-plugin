...
1. **Resolve workflow path** (override-first pattern):
   - Check if @.slide-builder/workflows/plan-one/instructions.md exists
   - If yes: Use @.slide-builder/workflows/plan-one/ as workflow root
   - If no: Use @${CLAUDE_PLUGIN_ROOT}/workflows/plan-one/ as workflow root
   - If neither exists: Display error "Workflow 'plan-one' not found in plugin or user overrides"
...
5. Save plan to output/singles/plan.yaml
6. Update .slide-builder/status.yaml with mode: "single"
</steps>
