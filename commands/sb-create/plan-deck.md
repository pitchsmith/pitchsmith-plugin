...
1. **Resolve workflow path** (override-first pattern):
   - Check if @.slide-builder/workflows/plan-deck/instructions.md exists
   - If yes: Use @.slide-builder/workflows/plan-deck/ as workflow root
   - If no: Use @${CLAUDE_PLUGIN_ROOT}/workflows/plan-deck/ as workflow root
   - If neither exists: Display error "Workflow 'plan-deck' not found in plugin or user overrides"
...
5. Each prompt MUST wait for user response before proceeding
6. Slide breakdown MUST show: number, intent, template, storyline_role for each slide
</steps>
