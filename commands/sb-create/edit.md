...
1. **Resolve workflow path** (override-first pattern):
   - Check if @.slide-builder/workflows/edit/instructions.md exists
   - If yes: Use @.slide-builder/workflows/edit/ as workflow root
   - If no: Use @${CLAUDE_PLUGIN_ROOT}/workflows/edit/ as workflow root
   - If neither exists: Display error "Workflow 'edit' not found in plugin or user overrides"
