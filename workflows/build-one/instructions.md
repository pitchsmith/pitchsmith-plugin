...
- {{this.id}}: {{this.description}}
  ...
  {{#if this.instructions}}Instructions: {{this.instructions}}{{/if}}
{{/each}}
...
<steps>
1. Set visual audit input variables from current build state:
   - `{{output_path}}` = path to the generated HTML slide file
   - `{{slide_number}}` = `{{slide.number}}` (current slide number)
   - `{{background_mode}}` = `{{slide.background_mode}}` or infer from theme (dark/light)
   - `{{storyline_role}}` = `{{slide.storyline_role}}` (opening, tension, resolution, evidence, cta)
</steps>
