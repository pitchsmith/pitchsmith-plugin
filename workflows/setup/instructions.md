...
**MCP availability check:** Before processing PDFs, attempt to load the tool via ToolSearch. If the tool is not found, display the following guidance message:

📄 **PDF Reading Requires MCP Server**

The pdf-reader MCP server is needed to extract content from PDF files. This is an optional dependency — you can still set up your brand using a website URL or image files.

**To enable PDF reading**, add this to your Claude Code MCP settings (`~/.claude/mcp_settings.json`):
...
```json
{
  "mcpServers": {
    "pdf-reader": {
      "command": "npx",
      "args": ["-y", "@nicobailon/mcp-pdf-reader"]
    }
  }
}
```
...
- A website URL for brand extraction
- Image files (logos, screenshots, brand guideline images)

Ask the user how they would like to proceed:
- **Website URL** — provide a website URL for brand extraction
- **Image files** — provide logo or screenshot images instead
- **Cancel** — stop setup and configure pdf-reader MCP first

If the user chooses an alternative input, continue the workflow with that input type. If the user has no other brand sources besides the PDF and chooses Cancel, stop the workflow gracefully after showing this message. If they provided other sources (website URL, images), skip the PDF step and continue with those.
</critical>
...
See `.slide-builder/reference/theme-schema.md#brandContext` for the expected data structure, field descriptions, and examples.
</reference>
...
<critical>
When building the theme.json structure, include the `brandContext` object composed in Step 2.9. The brandContext field stores unstructured brand context data that informs AI-driven slide generation. See `.slide-builder/reference/theme-schema.md#brandContext` for the expected data structure.
</critical>
...
<critical>
Validate the generated theme against the required schema fields before saving. This ensures every theme.json meets quality standards. Reference: `.slide-builder/reference/theme-schema.md` for authoritative field documentation.
</critical>
...
## Phase 3.5: Brand Asset Collection
...
<ask header="Logo Import" context="Your theme is ready! Before generating sample slides, you can import your brand logos so they appear in presentations.

Logos typically have dark and light variants for use on different slide backgrounds.
...
<check if="user selected 'Yes, I have logos'">

For each logo the user provides:

1. Ask for the file path(s)
2. Validate files exist and are a supported format (PNG, JPG, SVG, WebP)
3. Use Read tool to display the file for visual confirmation
4. Generate suggested logo ID from filename
5. Ask user for logo metadata:
   - Display name
   - Description
   - Variant ID (e.g., "dark", "light", "color") and usage description
...
### Step 3.5.3: Ask About Icon Files

<ask header="Icon Import" context="Icons are small symbolic graphics used for visual emphasis on slides.

For best results, icons should have both dark (for light backgrounds) and white (for dark backgrounds) variants.
...
<check if="user selected 'Yes, I have icons'">

For each icon the user provides:

1. Ask for the file path(s)
2. Validate files exist and are a supported format (PNG, SVG)
3. Use Read tool to display the file for visual confirmation
4. Generate icon ID from filename or ask user to specify
5. Ask user for metadata:
   - Display name
   - Description
...
```json
{
  ...
  "file": "{{filename}}",
  "size": {{size}},
  "backgroundAffinity": "{{light_or_dark}}",
  "hasTransparency": true,
  "dominantColors": ["{{color_1}}"],
  "contrastNeeds": "high",
  "tags": ["{{tag_1}}", "{{tag_2}}", "..."]
}
```
...
### Step 3.5.5: Handle Empty Catalogs

<check if="user skipped both logos and icons">
...
### Step 3.5.6: Asset Collection Summary

Report brand asset catalog status:
...
| content | Content Slide | general, informational, text-heavy | body text, headings, spacing, light background |
</reference>

### Step 4.1.5: Scan Brand Assets for Template Designs

Before generating slides, scan the brand assets collected in Phase 1 for existing slide-like designs that can inform template generation.

<critical>
This step uses assets already collected — do NOT re-prompt the user for inputs.
</critical>

1. **Identify slide-like designs in collected assets:**
   - Review any PDF pages that contain presentation slides, pitch decks, or slide-like layouts
   - Review any images that show slide designs, deck screenshots, or presentation mockups
   - Look for: title slides, content layouts, comparison layouts, process diagrams, callout/stat highlights

2. **Extract visual patterns from found designs:**
   - Layout structure (content placement, whitespace usage, grid patterns)
   - Typography hierarchy as actually used (heading sizes relative to body, weight choices)
   - Color application patterns (which colors used for backgrounds vs accents vs text)
   - Decorative elements (dividers, shapes, icons, gradients, borders)
   - Content density and spacing preferences

3. **Map found designs to catalog templates:**
   - For each of the 6 catalog templates (title, content, callout, agenda, comparison, process-flow), check if a matching design was found in the brand assets
   - If a match exists, note the specific visual patterns to replicate in the generated template
   - If no match exists for a template type, it will be generated purely from theme primitives

4. **Store design references for Step 4.2:**
   - Create a brief design-reference note for each template that has a brand asset match
   - Include: source asset (PDF page N / image filename), layout description, key visual elements to replicate
   - These references will be used as additional context when generating each slide in Step 4.2

<reference title="Example design reference">
| Template | Source | Key Visual Elements |
|----------|--------|-------------------|
| title | brand-guide.pdf page 12 | Centered headline, large logo top-right, gradient overlay on dark background |
| comparison | pitch-deck.png | Two-column with colored headers, icon bullets, subtle divider line |
| (others) | No match — generate from theme primitives | — |
</reference>
...
Check if existing decks exist in `output/` directory:

<check if="output directory contains deck folders">
  <output>
...
⚠️ **Important: Existing slides use the old theme**
...
<check if="output directory is empty or does not exist">
  <output>
✅ Brand theme extracted and installed
