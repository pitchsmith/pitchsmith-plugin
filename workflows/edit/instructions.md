...
Preview: Open the slide in your browser to see changes.
  ...
  <!-- ═══════════════════════════════════════════════════════════════════════ -->
  <!-- PHASE 5D: Visual Audit                                                  -->
  <!-- ═══════════════════════════════════════════════════════════════════════ -->
  <step n="5D" goal="Validate regenerated slide visually in a real browser">
    <context>
    After saving the regenerated slide, run the shared visual audit protocol to catch
    rendering issues (CSS variables, fonts, layout overflow, icon contrast) before
    asking the user about further edits. Non-blocking: always continues to Phase 6.
    </context>

    <action>Set visual audit input variables:
      - `{{output_path}}` = `{{target_slide_path}}`
      - `{{slide_number}}` = current slide number from workflow state
      - `{{background_mode}}` = infer from theme or slide state (dark/light)
    ...
    <check if="{{visual_audit_skipped}} == false AND {{visual_audit_fixes_applied}} > 0">
      <action>Re-read `{{target_slide_path}}` to pick up any auto-fix changes applied by the protocol</action>
    </check>
  </step>
