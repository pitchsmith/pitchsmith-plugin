...
function executeMcpTool(source, template_context):
    ...
    loadResult = ToolSearch("select:" + toolName)
    if toolName not found:
        // MCP server not configured — provide install guidance
        log("📄 MCP tool not available: " + toolName)
        if toolName == "mcp__pdf-reader__read_pdf":
            display guidance:
                📄 **PDF Reading Requires MCP Server**

                The pdf-reader MCP server is needed to extract content from PDF files.
                This is an optional dependency — you can enter content manually or skip this field.
                ...
                **Alternatively, you can:**
                ...
                - Skip this content field (placeholder content will be used)

            ask user how to proceed:
                - **Enter manually** — type or paste the content for this field
                - **Skip** — use placeholder content and continue

            if user chooses "Enter manually":
                return executeUserInput({type: "user_input", field: source.field, prompt: "Please provide the content for '" + source.field + "' (original PDF source unavailable)"}, template_context)
            if user chooses "Skip":
                return { success: true, data: { [source.field]: "[Placeholder — PDF content not available. Configure pdf-reader MCP and rebuild.]" }, source_type: "mcp_tool", usedFallback: true }
        return { success: false, error: "MCP tool not available", source_type: "mcp_tool" }
...
<step n="5" goal="Generate manifest, viewer, and finalize status">
  <action>Run generate-manifest.js to generate manifest.json:
    ```
    node ${CLAUDE_PLUGIN_ROOT}/scripts/generate-manifest.js {{deck_slug}}
    ```
    This script:
  ...
  <check if="generate-manifest.js fails">
    <output>⚠️ Manifest generation failed. Slides are saved but manifest.json was not updated.
You can retry with: `node ${CLAUDE_PLUGIN_ROOT}/scripts/generate-manifest.js {{deck_slug}}`</output>
  </check>
