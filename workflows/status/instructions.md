...
{{end for}}
    ...
    <!-- Workflow Configuration Section (AC-1 through AC-8) -->
    <action>Scan the `.slide-builder/workflows/` directory for subdirectories. If the directory does not exist or is empty, set ejected_workflows to an empty list. Only include entries that are directories (ignore files). Each subdirectory name represents an ejected workflow name.</action>

    <action>Define the known plugin workflow list (21 workflows, excluding `shared/`):
      `add-deck-template`, `add-slide`, `add-slide-template`, `animate`, `build`, `build-all`, `build-one`, `delete-deck`, `edit`, `edit-deck-template`, `edit-plan`, `optimize-instructions`, `plan`, `plan-deck`, `plan-one`, `setup`, `status`, `theme`, `theme-edit`, `update-brand-assets`, `use-template-deck`
    </action>

    <action>Compute:
      - ejected_count = number of ejected workflow directories found
      - total_workflows = 21
      - default_workflows = known workflows NOT present in ejected_workflows list
    </action>

    <check if="ejected_count == 0">
      <output>
---
...
> **Tip:** Run `/sb-manage:eject-workflow {name}` to customize a workflow
    ...
    <check if="ejected_count > 0">
      <output>
---
**Workflow Configuration:** {{ejected_count}} of {{total_workflows}} workflows customized

**Using local customizations:**
{{for each wf in ejected_workflows:}}
- {{wf}} (`.slide-builder/workflows/{{wf}}/`)
{{end for}}

**Using plugin defaults:**
{{for each wf in default_workflows:}}
- {{wf}}
{{end for}}
