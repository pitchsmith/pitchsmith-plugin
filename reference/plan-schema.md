...
# Presentation Settings (optional - from adaptive discovery Phase 2.5/2.6)
presentation_setting: "string" # Optional: live | sent_as_deck | recorded | hybrid
desired_length: "string"       # Optional: Target slide count, e.g. "8-10" or "12"

# Discovery Context (optional - from adaptive follow-ups Phase 2.3)
discovery_context:             # Optional: Flexible key-value pairs from follow-up answers
  # Actual keys depend on what gaps were identified during discovery.
  # Common keys include:
  known_objections: ["string"]   # e.g., ["Too expensive", "Long implementation"]
  competitive_context: "string"  # e.g., "Replacing incumbent Vendor X"
  audience_priorities: ["string"] # e.g., ["ROI", "time-to-value"]
  audience_role: "string"        # e.g., "decision_maker", "technical_evaluator"
  decision_timeline: "string"    # e.g., "Q4 budget decision"
...
<reference title="Optional deck-level fields">
...
| desired_length | string | Range "8-10" or single "12" | Target slide count from Phase 2.6 length confirmation |
| discovery_context | object | Flexible key-value pairs | Context from adaptive follow-ups (see Discovery Context below) |
| planning_research | array | Array of research objects | Web research findings from Phase 2.4 |
</reference>

<reference title="Discovery context common keys">
...
| audience_role | string | Role of the primary audience (e.g., "decision_maker", "technical_evaluator") |
| decision_timeline | string | When the audience needs to make a decision |

Note: `discovery_context` is a flexible object. The keys above are common examples, not a rigid schema. Actual keys depend on what gaps were identified during discovery. Do not validate against a fixed set of keys.
</reference>
...
Plans created before Epic AD-1 will not contain `presentation_setting`, `desired_length`, or `discovery_context` fields. These fields are optional — their absence must not cause errors in any downstream workflow (build-one, edit, etc.). No migration is required.
</important>
...
# Optional: Presentation settings from adaptive discovery
presentation_setting: "live"
desired_length: "8-10"

# Optional: Discovery context from adaptive follow-ups
discovery_context:
...
- Optional `presentation_setting` and `desired_length` from adaptive discovery
- Optional `discovery_context` with flexible key-value pairs from follow-ups
